resource :ecstasklong, 'AWS::ECS::TaskDefinition', DependsOn: [:iamroleexec, :iamroletask, :loggroup] do
  cpu 512
  memory '2GB'
  ...
  container_definitions [
    {
      ...
      Command: %w[divvycloudworker -t harvester-long -n],
      LogConfiguration: {
        LogDriver: :awslogs,
        Options: {
          'awslogs-region'        => Fn::ref('AWS::Region'),
          'awslogs-group'         => Fn::ref(:loggroup),
          'awslogs-stream-prefix' => :longharvest,
        }
      }
    }
  ]
